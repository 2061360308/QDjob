...
3. **获取书籍ID和章节ID**，在起点官网上找到你要上报时长的书籍，查看其网址  
   提取书籍ID: 
   ![bookid](picture/bookid.png)
   提取书籍ID和章节ID: 
   ![chapterid](picture/chapterid.png)
4. **配置上报数据**，打开`QDjob_editor`，选中用户，打开阅读时长上报界面，将上面的书籍ID和章节ID填入，配置阅读时长和阅读结束时间，程序会自动计算阅读开始时间，添加记录。**建议单次上报时至少上报2条记录**，否则会影响时间统计。
5. **点击确认上报**  
